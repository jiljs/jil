...
export function getFixturePath(fixture: string, file = ''): string {
  return normalizePath(FIXTURES_DIR, fixture, file);
}

export function getFixtureNodeModulePath(fixture: string, moduleName: string, file = ''): string {
  return normalizePath(FIXTURES_DIR, fixture, 'node_modules', moduleName, file);
}

export function getNodeModulePath(moduleName: string, file = ''): string {
...
export function copyFixtureToNodeModule(fixture: string, moduleName: string, customModulePath = false): () => void {
...
export function createTempFixtureFolder(): string {
  const tmp = os.tmpdir();
  const dir = normalizePath(tmp, `jil-${Date.now().toString(16)}${folderCount}`);
