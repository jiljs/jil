...
import {LoggerFunction} from '@jil/log';
import {CLIError} from './CLIError';
import {CommandManager} from './CommandManager';
import {INTERNAL_INITIALIZER, INTERNAL_OPTIONS, INTERNAL_PARAMS, INTERNAL_PROGRAM} from './constants';
  ...
  implements Commandable<O, P>
{
  ...
  static allowUnknownOptions = false;
   ...
   * and pass the results through a promise. This does *not* execute Jil CLI
  ...
  getParserOptions(): ParserOptions<O, P> {
    const {aliases, allowUnknownOptions, allowVariadicParams, options, params, path} = this.getMetadata();
   ...
   */
  private handleBeforeRegister = ([subPath]: [CommandPath, unknown]) => {
