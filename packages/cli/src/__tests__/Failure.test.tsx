...
describe('<Failure />', () => {
	it('renders a common error', async () => {
		await expect(
			renderComponent(<Failure binName="jil" error={new Error('Something is broken!')} />),
	...
	it('doesnt render code frame if arg not found', async () => {
		...
		await expect(
			renderComponent(<Failure binName="jil" commandLine="foo --bar" error={error} />),
		).resolves.toMatchSnapshot();
	});
	...
	it('renders an error with warnings', async () => {
		...
		const out = await renderComponent(
			<Failure binName="jil" error={new Error('Something is broken!')} warnings={warnings} />,
		);
	...
	it('renders a parse error', async () => {
		...
		await expect(renderComponent(<Failure binName="jil" error={error} />)).resolves.toMatchSnapshot();
	});
	...
	it('renders a parse error with a command line', async () => {
		...
		await expect(
			renderComponent(
				<Failure binName="jil" commandLine="--foo value --flag=123 -gSA" error={error} />,
				true,
			),
		).resolves.toMatchSnapshot();
	});
	...
	it('reduces command line by half when index appears off screen', async () => {
		...
		await expect(
			renderComponent(<Failure binName="jil" commandLine={line} error={error} />, true),
		).resolves.toMatchSnapshot();
	});
});
