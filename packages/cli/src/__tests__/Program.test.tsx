...
import { ProgramContext, useProgram } from "../react";
import { MockReadStream, MockWriteStream, runProgram, runTask } from "../mocks";
...
describe("<Program />", () => {
  let originalUnexpectedErrorHandler: any;
  let program: Program;
  ...
  describe("commands", () => {
    ...
    it("emits `onBeforeRegister` and `onAfterRegister` events", () => {
      ...
      program.onBeforeRegister(beforeSpy);
      program.onAfterRegister(afterSpy);
  ...
  describe("help", () => {
    ...
    it("emits `onHelp` event", async () => {
      ...
      await runProgram(program, ["jil", "--help"]);
  ...
  describe("failure", () => {
    ...
    it("renders if an error is thrown", async () => {
      ...
      const { code, output } = await runProgram(program, ["jil"]);
    ...
    it("emits `onBeforeRun` and `onAfterRun` events", async () => {
      ...
      program.onBeforeRun(beforeSpy);
      program.onAfterRun(afterSpy);

      program.register(new ErrorCommand());

      await runProgram(program, ["jil"]);

      expect(beforeSpy).toHaveBeenCalledWith(["jil"]);
  ...
  describe("success", () => {
    ...
    it("emits `onBeforeRun` and `onAfterRun` events", async () => {
      ...
      await runProgram(program, ["jil", "foo", "bar"]);

      expect(beforeSpy).toHaveBeenCalledWith(["jil", "foo", "bar"]);
    ...
    it("emits `onCommandFound` event", async () => {
      ...
      expect(spy).toHaveBeenCalledWith([["jil", "foo", "bar"], "jil", cmd]);
    });
    ...
    it("emits `onBeforeRender` and `onAfterRender` events for components", async () => {
      ...
      program.onBeforeRender(beforeSpy);
      program.onAfterRender(afterSpy);
      program.default(new ComponentCommand());
