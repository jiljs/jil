 ...
 * This does not mock all stream functionality, only those required by Jil and Ink.
 *
 * ```ts
 * import { mockStreams } from '@jil/cli/mocks';
 *
 ...
 *
 * ```ts
 * import { mockProgram } from '@jil/cli/mocks';
 *
 ...
 *
 * ```tsx
 * import { renderComponent } from '@jil/cli/mocks';
 ...
 */
export async function renderComponent(element: React.ReactElement, stripped = false): Promise<string> {
  let output = '';

  try {
    const {render: renderAsTest} = await import('ink-testing-library');
    const {lastFrame} = renderAsTest(element);
 ...
 *
 * ```ts
 * import { runCommand } from '@jil/cli/mocks';
 ...
 *
 * ```ts
 * import { runTask } from '@jil/cli/mocks';
 ...
 *
 * ```ts
 * import { runProgram } from '@jil/cli/mocks';
