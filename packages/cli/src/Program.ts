...
import {createLogger, formats, LoggerFunction, StreamTransport} from '@jil/log';
...
export class Program extends CommandManager<ProgramOptions> {
  ...
  protected _onAfterRender = new Emitter();
  /**
   * Called after a component has rendered.
   * @category Events
   */
  readonly onAfterRender = this._onAfterRender.event;
  protected _onAfterRun = new Emitter<Error | undefined>();
  /**
   * Called after the program and command have been ran.
   * @category Events
   */
  readonly onAfterRun = this._onAfterRun.event;
  protected _onBeforeRender = new Emitter<RunResult>();
  /**
   * Called after a command has run but before a component will render.
   * @category Events
   */
  readonly onBeforeRender = this._onBeforeRender.event;
  protected _onBeforeRun = new Emitter<Argv>();
  /**
   * Called before the program and command will run.
   * @category Events
   */
  readonly onBeforeRun = this._onBeforeRun.event;
  protected _onCommandFound = new Emitter<[Argv, CommandPath, Commandable]>();
  /**
   * Called when a command has been found after parsing argv.
   * @category Events
   */
  readonly onCommandFound = this._onCommandFound.event;
  protected _onCommandNotFound = new Emitter<[Argv, CommandPath]>();
  /**
   * Called when a command wasn't found after parsing argv.
   * @category Events
   */
  readonly onCommandNotFound = this._onCommandNotFound.event;
  protected _onExit = new Emitter<[string, ExitCode]>();
  /**
   * Called when the exit() handler is executed but before the process exits.
   * @category Events
   */
  readonly onExit = this._onExit.event;
  protected _onHelp = new Emitter<CommandPath | undefined>();
  /**
   * Called when the help menu is rendered.
   * @category Events
   */
  readonly onHelp = this._onHelp.event;
  ...
  constructor(options: ProgramOptions, streams?: ProgramStreams) {
    ...
    this.onAfterRegister(this.handleAfterRegister);
    this.onBeforeRegister(this.handleBeforeRegister);
  }
  ...
  exit = (error?: Error | string, errorCode?: ExitCode) => {
    ...
    this._onExit.emit([message, code]);
  ...
  parse<O extends GlobalOptions, P extends PrimitiveType[] = ArgList>(argv: Argv): Arguments<O, P> {
    ...
    try {
      ...
      this._onCommandNotFound.emit([argv, possibleCmd]);
  ...
  async renderElement(element: React.ReactElement) {
    ...
    try {
      this._onBeforeRender.emit(element);
   ...
   * - Return exit code.
   */
  async run(argv: Argv, bootstrap?: ProgramBootstrap, rethrow = false): Promise<ExitCode> {
    this._onBeforeRun.emit(argv);
  ...
  protected async createIndex(): Promise<React.ReactElement> {
    ...
    if (this.standAlone) {
      return createElement(IndexHelp, this.options, await this.getCommand(this.standAlone)!.createHelp());
    }
  ...
  protected async runAndRender(argv: Argv): Promise<ExitCode> {
    ...
    // Parse the arguments
    const {command: paths, errors, options, params, rest, unknown} = await this.applyMiddlewareAndParseArgs(argv);
    const path = paths.join(':') || this.standAlone;
    const command = this.getCommand(path) as Command;

    this._onCommandFound.emit([argv, path, command]);
   ...
   */
  private handleAfterRegister = ([, command]: [CommandPath, Commandable]) => {
