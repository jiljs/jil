...
import {DEFAULT_PRIORITY, Registration, Registry} from '..';
...
describe('Registry', () => {
  ...
  it('sets correct properties', () => {
    expect(registry.projectName).toBe('jil-test');
  ...
  describe('formatModuleName()', () => {
    it('allows a custom name', () => {
      expect(registry.formatModuleName('bar')).toBe('jil-test-renderer-bar');
    });
    ...
    it('supports dashes', () => {
      expect(registry.formatModuleName('bar-baz')).toBe('jil-test-renderer-bar-baz');
    });

    it('supports scoped', () => {
      expect(registry.formatModuleName('bar', true)).toBe('@jil-test/renderer-bar');
    });
  });
  ...
  describe('get()', () => {
    ...
    beforeEach(async () => {
      await registry.register('jil-test-renderer-foo', foo, tool);
      await registry.register('@jil-test/renderer-bar-baz', barBaz, tool);
      await registry.register('@test/jil-test-renderer-qux', qux, tool);
    });
    ...
    it('returns plugin by qualified public name', () => {
      expect(registry.get('jil-test-renderer-foo')).toBe(foo);
    });

    it('returns plugin by qualified internal name', () => {
      expect(registry.get('@jil-test/renderer-bar-baz')).toBe(barBaz);
    });

    it('returns plugin by qualified custom scope name', () => {
      expect(registry.get('@test/jil-test-renderer-qux')).toBe(qux);
    });
  });
  ...
  describe('getAll()', () => {
    ...
    beforeEach(async () => {
      await registry.register('jil-test-renderer-foo', foo, tool);
      await registry.register('jil-test-renderer-bar', bar, tool);
      await registry.register('jil-test-renderer-baz', baz, tool);
    });
  ...
  describe('getMany()', () => {
    ...
    beforeEach(async () => {
      await registry.register('jil-test-renderer-foo', foo, tool);
      await registry.register('@jil-test/renderer-bar', bar, tool);
      await registry.register('@test/jil-test-renderer-baz', baz, tool);
    });
    ...
    it('returns all by qualified name', () => {
      expect(registry.getMany(['@test/jil-test-renderer-baz', '@jil-test/renderer-bar'])).toEqual([baz, bar]);
    });

    it('returns all by mixed names', () => {
      expect(
        registry.getMany([
          '@test/jil-test-renderer-baz',
          'foo',
          '@jil-test/renderer-bar',
          'bar',
          'jil-test-renderer-foo',
        ]),
  ...
  describe('loadMany()', () => {
    ...
    it('emits `onLoad` event for each plugin', async () => {
      fixtures.push(
        copyFixtureToNodeModule('plugin-renderer-object', 'jil-test-renderer-foo'),
        copyFixtureToNodeModule('plugin-renderer-object', '@jil-test/renderer-bar'),
        copyFixtureToNodeModule('plugin-renderer-object', '@test/jil-test-renderer-baz'),
      );
    ...
    it('loads plugins based on source', async () => {
      ...
      expect(plugins).toEqual([
        ...
        {
          name: '@test/jil-test-renderer-baz',
          plugin: expect.any(Object),
          priority: DEFAULT_PRIORITY,
        },
        {
          name: 'jil-test-renderer-foo',
          plugin: expect.any(Object),
          priority: DEFAULT_PRIORITY,
        },
        {
    ...
    it('loads plugins using a tuple with options', async () => {
      fixtures.push(
        copyFixtureToNodeModule('plugin-renderer-object', '@test/jil-test-renderer-baz-scoped'),
        copyFixtureToNodeModule('plugin-renderer-object', 'jil-test-renderer-foo-short'),
        copyFixtureToNodeModule('plugin-renderer-object', '@jil-test/renderer-bar-short'),
        copyFixtureToNodeModule('plugin-renderer-object', 'jil-test-renderer-foo-full'),
        copyFixtureToNodeModule('plugin-renderer-object', '@jil-test/renderer-bar-full'),
      );
      ...
      await registry.loadMany(
        [
          ...
          'bar-short',
          // Full names
          'jil-test-renderer-foo-full',
          ['@jil-test/renderer-bar-full', {value: 'bar-full'}],
          // Full name with custom scope
          '@test/jil-test-renderer-baz-scoped',
      ...
      expect(plugins).toEqual([
        {
          name: '@jil-test/renderer-bar-full',
        ...
        {
          name: '@jil-test/renderer-bar-short',
          plugin: expect.any(Object),
          priority: DEFAULT_PRIORITY,
        },
        {
          name: '@test/jil-test-renderer-baz-scoped',
          plugin: expect.any(Object),
          priority: DEFAULT_PRIORITY,
        },
        {
          name: 'jil-test-renderer-foo-full',
          plugin: expect.any(Object),
          priority: DEFAULT_PRIORITY,
        },
        {
          name: 'jil-test-renderer-foo-short',
          plugin: expect.objectContaining({
    ...
    it('loads plugins using an object with options', async () => {
      fixtures.push(
        copyFixtureToNodeModule('plugin-renderer-object', 'jil-test-renderer-foo3'),
        copyFixtureToNodeModule('plugin-renderer-object', '@jil-test/renderer-bar3'),
        copyFixtureToNodeModule('plugin-renderer-object', '@test/jil-test-renderer-baz3'),
      );
      ...
      await registry.loadMany(
        {
          ...
          foo3: {value: 'foo3'},
          // Full names
          '@jil-test/renderer-bar3': {value: 'bar3'},
          // Full name enabled
          '@test/baz3': true,
          // Short name disabled
          qux: false,
      ...
      expect(plugins).toEqual([
        ...
        {
          name: '@test/jil-test-renderer-baz3',
          plugin: expect.objectContaining({
            options: {},
          }),
          priority: 100,
    ...
    it('uses objects as is', async () => {
      ...
      await registry.loadMany(
        [
          // Basic
          {name: 'jil-test-renderer-foo', render},
          // With options
          // @ts-expect-error Invalid options
          {name: '@jil-test/renderer-bar', options: {value: 'bar'}, render},
          // With options and priority
          {name: '@test/jil-test-renderer-baz', options: {value: 'baz'}, priority: 1, render},
        ],
        {},
      );
    ...
    it('uses class instances as is', async () => {
      // Basic
      const foo = createClass('jil-test-renderer-foo');
      // With options
      const bar = createClass('@jil-test/renderer-bar', {value: 'bar'});
      // With options and custom scope
      const baz = createClass('@test/jil-test-renderer-baz', {value: 'baz'});
      baz.priority = 1;
  ...
  describe('unregister()', () => {
    ...
    it('emits `onAfterUnregister` event', async () => {
      ...
      registry.onAfterUnregister(spy);
    ...
    it('emits `onBeforeUnregister` event', async () => {
      ...
      registry.onBeforeUnregister(spy);
