...
export class Registry<Plugin extends Pluggable, Tool = unknown> extends Contract<RegistryOptions<Plugin>> {
  readonly debug: Debugger;
  readonly pluralName: string;
  readonly projectName: string;
  readonly singularName: string;

  protected _onAfterRegister = new Emitter<Plugin>();
  /**
   * Called after a plugin is registered.
   * @category Events
   */
  readonly onAfterRegister = this._onAfterRegister.event;
  protected _onAfterUnregister = new Emitter<Plugin>();
  /**
   * Called after a plugin is unregistered.
   * @category Events
   */
  readonly onAfterUnregister = this._onAfterUnregister.event;
  protected _onBeforeRegister = new Emitter<Plugin>();
  /**
   * Called before a plugin is registered and the `startup` life-cycle.
   * @category Events
   */
  readonly onBeforeRegister = this._onBeforeRegister.event;
  protected _onBeforeUnregister = new Emitter<Plugin>();
  /**
   * Called before a plugin is unregistered and the `shutdown` life-cycle.
   * @category Events
   */
  readonly onBeforeUnregister = this._onBeforeUnregister.event;
  protected _onLoad = new Emitter<[string, object]>();
  /**
  ...
  constructor(projectName: string, typeName: string, options: RegistryOptions<Plugin>) {
    ...
    this.projectName = kebabCaseName(projectName);
    this.singularName = kebabCaseName(typeName);
   ...
   */
  formatModuleName(name: string, scoped = false): ModuleID {
    if (scoped) {
   ...
   */
  async load(source: Plugin | Source, params: object = {}, options: RegisterOptions<Tool> = {}): Promise<Plugin> {
    let plugin: Plugin;
    ...
    if (isObject(source)) {
      ...
      this._onLoad.emit([source, params]);
  ...
  async register(
    ...
    this._onBeforeRegister.emit(plugin);
    ...
    this._onAfterRegister.emit(plugin);
  ...
  async unregister(name: ModuleID, tool?: Tool): Promise<Plugin> {
    ...
    this._onBeforeUnregister.emit(plugin);
    ...
    this._onAfterUnregister.emit(plugin);
