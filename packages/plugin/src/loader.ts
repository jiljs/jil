...
export class Loader<Plugin extends Pluggable> {
  ...
  async load(source: Source, options: object = {}): Promise<Plugin> {
    const {originalSource, resolvedPath} = await this.createResolver(source).resolve(__dirname);

    this.debug('Loading %s from %s', color.moduleName(originalSource), color.filePath(resolvedPath));
