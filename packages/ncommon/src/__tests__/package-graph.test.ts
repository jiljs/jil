...
import {mockFilePath} from '../mocks';
import {PackageStructure} from '../types';

// eslint-disable-next-line @typescript-eslint/no-unused-vars
function getScopedPackages(scope: string) {
  ...
  glob
    .sync('*/package.json', {
      absolute: true,
      cwd: mockFilePath(__dirname, `../../../node_modules/@${scope}`).path(),
    })
    .forEach(pkgPath => {
          ...
          nodes: [
            {
              ...
              nodes: [{package: {name: 'foo', version: '0.0.0', dependencies: {bar: '0.0.0'}}}],
            },
          ],
        },
      ],
