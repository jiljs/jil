...
import {mockFilePath, normalizeSeparators} from '../mocks';
...
describe('Project', () => {
  ...
  describe('getWorkspaceGlobs()', () => {
    ...
    it('returns an array of workspaces for pnpm `packages` property', () => {
      expect(new Project(getFixturePath('workspace-pnpm')).getWorkspaceGlobs({relative: true})).toEqual(
        ['packages/**', '!**/qux'].map(Project.normalizeGlob),
      );
    });

    it('returns an array of all workspaces', () => {
      expect(new Project(getFixturePath('workspace-multiple')).getWorkspaceGlobs()).toEqual(
        [getFixturePath('workspace-multiple', 'packages/*'), getFixturePath('workspace-multiple', 'modules/*')].map(
          Project.normalizeGlob,
        ),
      );
    });

    it('returns an array of all workspaces as relative paths', () => {
      expect(new Project(getFixturePath('workspace-multiple')).getWorkspaceGlobs({relative: true})).toEqual(
        ['packages/*', 'modules/*'].map(Project.normalizeGlob),
      );
    });
  });
  ...
  describe('getWorkspacePackages()', () => {
    ...
    it('loads all package.jsons and appends metadata', () => {
      ...
      expect(project.getWorkspacePackages()).toEqual([
        {
          package: {name: 'test-jil-workspace-multiple-baz', version: '0.0.0'},
          metadata: project.createWorkspaceMetadata(rootPath.append(normalizeSeparators('packages/baz/package.json'))),
        },
        {
          package: {name: 'test-jil-workspace-multiple-foo', version: '0.0.0'},
          metadata: project.createWorkspaceMetadata(rootPath.append(normalizeSeparators('packages/foo/package.json'))),
        },
        {
          package: {name: 'test-jil-workspace-multiple-bar', version: '0.0.0'},
          metadata: project.createWorkspaceMetadata(rootPath.append(normalizeSeparators('modules/bar/package.json'))),
        },
      ]);
    });
    ...
    it("loads package.json's for yarn", () => {
      ...
      expect(project.getWorkspacePackages()).toEqual([
        {
          package: {name: 'test-jil-workspace-foo-yarn', version: '0.0.0'},
          metadata: project.createWorkspaceMetadata(rootPath.append(normalizeSeparators('packages/foo/package.json'))),
        },
      ]);
    });
    ...
    it("loads package.json's for lerna", () => {
      ...
      expect(project.getWorkspacePackages()).toEqual([
        {
          package: {name: 'test-jil-workspace-foo-lerna', version: '0.0.0'},
          metadata: project.createWorkspaceMetadata(rootPath.append(normalizeSeparators('packages/foo/package.json'))),
        },
      ]);
    });
    ...
    it("loads package.json's for pnpm", () => {
      ...
      expect(project.getWorkspacePackages()).toEqual([
        {
          package: {name: 'test-jil-workspace-foo-pnpm', version: '0.0.0'},
          metadata: project.createWorkspaceMetadata(rootPath.append(normalizeSeparators('packages/foo/package.json'))),
        },
      ]);
    });
  ...
  describe('getWorkspacePackagePaths()', () => {
    it('returns an empty array if no workspace config', () => {
      expect(new Project(getFixturePath('workspace-no-packages')).getWorkspacePackagePaths()).toEqual([]);
    });
    ...
    it('returns an array of all packages within all workspaces as relative paths', () => {
      ...
      ).toEqual([mockFilePath('packages/baz'), mockFilePath('packages/foo'), mockFilePath('modules/bar')]);
    });
  });
});
