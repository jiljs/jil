...
if (!Mode.prototype.setAead) {
  ...
  cipher.encrypt = (name: string, key: Buffer, iv: Buffer, data: Buffer, tagLen?: number) =>
    encrypt(cipher.Cipher, name, key, iv, data, tagLen);

  cipher.decrypt = (name: string, key: Buffer, iv: Buffer, data: Buffer, tagOrLen?: number | Buffer) =>
