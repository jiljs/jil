...
import {BufferReader, StaticWriter, Struct} from '@jil/bufio';
...
export namespace asn1 {
  ...
  export class Node extends Struct {
    ...
    explicit(target: number) {
      expect(target >>> 0 === target).toBeTruthy();
      this.mode = EXPLICIT;
      this.target = target;
      return this;
    }
  ...
  export class Any extends Node {
    ...
    write(bw: bio.StaticWriter, extra?: any) {
      assert(bw);
      expect(this.mode === NORMAL).toBeTruthy();
      this.node.flags = this.flags;
  ...
  export class Choice extends Node {
    ...
    read(br: BufferReader, extra?: any) {
      ...
      expect(choices.length >= 1).toBeTruthy();
  ...
  export class OID extends Node {
    ...
    constructor(value?: number | number[] | Buffer | Uint32Array | string, ...options: any[]) {
      super();
    ...
    fromString(str: string) {
      ...
      str = oids.attrs[str] ?? oids.keyAlgs[str] ?? oids.hashes[str] ?? oids.curves[str] ?? str;
  ...
  export class UTCTime extends Time {
    ...
    writeBody(bw: bio.StaticWriter) {
      expect(isTime(this.value)).toBeTruthy();
  ...
  function sizeHeader(size: number) {
    ...
    expect(size <= 0xffffff).toBeTruthy();

    return 1 + 1 + 3;
  }

  function writeHeader(bw: bio.StaticWriter, type: number, cls: number, primitive: boolean, size: number) {
    assert(bw);
    expect(type >>> 0 === type).toBeTruthy();
    expect(cls >>> 0 === cls).toBeTruthy();
  ...
  function twosComplement(num: Buffer, start: number, end: number) {
    expect(Buffer.isBuffer(num)).toBeTruthy();
    expect(start >>> 0 === start).toBeTruthy();
    expect(end >>> 0 === end).toBeTruthy();
  ...
  function toSeconds(year: number, mon: number, day: number, hour: number, min: number, sec: number, utc: boolean) {
    expect(year >>> 0 === year).toBeTruthy();
    expect(mon >>> 0 === mon).toBeTruthy();
    expect(day >>> 0 === day).toBeTruthy();
    expect(hour >>> 0 === hour).toBeTruthy();
    expect(min >>> 0 === min).toBeTruthy();
    expect(sec >>> 0 === sec).toBeTruthy();
