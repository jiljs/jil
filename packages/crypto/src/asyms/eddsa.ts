...
class EDDSAAdsa implements Adsa {
  ...
  sign(msg: string | Buffer, key: any, ph: boolean | null, ctx: Buffer): Buffer {
    return this.signMessage(msg, key, ph, ctx);
  }

  signMessage(msg: string | Buffer, key: any, ph: boolean | null, ctx: Buffer): Buffer {
  ...
  signDigest(digest: string | Buffer, key: Buffer, ph: boolean | null, ctx: Buffer): Buffer {
    digest = Buffer.isBuffer(digest) ? digest : Buffer.from(digest, 'hex');
    return this.asym.sign(digest, key, ph, ctx);
  }

  verify(msg: string | Buffer, sig: Buffer, key: any, ph: boolean | null, ctx: Buffer): boolean {
    return this.verifyMessage(msg, sig, key, ph, ctx);
  }

  verifyMessage(msg: string | Buffer, sig: Buffer, key: any, ph: boolean | null, ctx: Buffer): boolean {
    assert(this.hash, 'hash is required');
    msg = Buffer.isBuffer(msg) ? msg : Buffer.from(msg, 'hex');
    return this.asym.verify(this.hash!.digest(msg), sig, key, ph, ctx);
  }

  verifyDigest(digest: string | Buffer, sig: Buffer, key: Buffer, ph: boolean | null, ctx: Buffer): boolean {
    digest = Buffer.isBuffer(digest) ? digest : Buffer.from(digest, 'hex');
