...
const adsables: Asym<any, any>[] = [ed448, ed25519, p192, p224, p256, p384, p521, secp256k1, rsa];
...
describe('ADSA', function () {
  for (const adsa of adsas) {
    it(`should generate keypair and sign RS (${adsa.id})`, function () {
      ...
      expect(adsa.verify(msg, sig, pub)).toBeTruthy();
      expect(adsa.verify(msg, sig, pubu)).toBeTruthy();
