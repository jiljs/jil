...
const curves = [p192, p224, p256, p384, p521, secp256k1, ed25519, ed448, x25519, x448];
...
describe('ECIES', function () {
  ...
  for (const curve of curves) {
    it(`should encrypt and decrypt (${curve.id})`, () => {
      ...
      expect(ct.length > msg.length).toBeTruthy();

      const pt = ecies.decrypt(curve, SHA256, ct, bobPriv);
      expect(pt).toEqual(msg);

      expect(() => ecies.decrypt(curve, SHA256, ct, alicePriv)).toThrow();

      ct[1] ^= 1;
      expect(() => ecies.decrypt(curve, SHA256, ct, bobPriv)).toThrow();
      ct[1] ^= 1;
    });
  }
