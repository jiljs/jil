...
describe('Bech32', function () {
  ...
  for (const [addr, script_] of validAddresses) {
    ...
    it(`should have valid address for ${text}`, () => {
      ...
      try {
        [hrp, version, hash] = decode(expected, addr);
      } catch (e) {
        hrp = null;
      }
      ...
      expect(hrp).not.toBeNull();
      expect(program(version!, hash!)).toEqual(script);
      expect(encode(hrp!, version!, hash!)).toEqual(addr.toLowerCase());
      expect(bech32.test(addr)).toBeTruthy();
    });
  }
  ...
  for (const addr of invalidAddresses) {
    ...
    it(`should have invalid address for ${text}`, () => {
      expect(() => decode('bc', addr)).toThrow();
      expect(() => decode('tb', addr)).toThrow();
    });
  }
  ...
  for (const [addr, script_] of validAddresses) {
    ...
    it(`should have valid address for ${text}`, () => {
      ...
      try {
        [hrp, version, hash] = decodeManual(expected, addr);
      } catch (e) {
        hrp = null;
      }
      ...
      expect(hrp).not.toBeNull();
      expect(program(version!, hash!)).toEqual(script);
      expect(encodeManual(hrp!, version!, hash!)).toEqual(addr.toLowerCase());
      expect(bech32.test(addr)).toBeTruthy();
      expect(bech32.is(addr)).toBeTruthy();
    });
  }
  ...
  for (const addr of invalidAddresses) {
    ...
    it(`should have invalid address for ${text}`, () => {
      expect(() => decodeManual('bc', addr)).toThrow();
      expect(() => decodeManual('tb', addr)).toThrow();
    });
  }
  ...
  for (const addr of invalidIs) {
    ...
    it(`should have invalid address for ${text}`, () => {
      expect(() => bech32.deserialize(addr)).toThrow();
      expect(bech32.is(addr)).toBe(false);
    });
  }
  ...
  for (const addr of invalidTest) {
    ...
    it(`should have invalid address for ${text}`, () => {
      expect(() => bech32.decode(addr)).toThrow();
      expect(bech32.test(addr)).toBe(false);
    });
  }
  ...
  for (const [hrp, version, hex, addr1] of vectors) {
    ...
    it(`should decode and re-serialize ${text}`, () => {
      // eslint-disable-next-line @typescript-eslint/naming-convention
      const [hrp_, version_, hash_] = bech32.decode(addr1);

      expect(hrp_).toBe(hrp);
      expect(version_).toBe(version);
      expect(hash_).toEqual(hash);

      const addr2 = bech32.encode(hrp, version, hash);

      expect(addr2).toEqual(addr1.toLowerCase());
    });
  ...
  it('should encode/decode random data', () => {
    for (let i = 20; i <= 50; i++) {
      ...
      const converted = bech32.convertBits(data, 8, 5, true);
      const str = bech32.serialize('bc', converted);
      const [, deserialized] = bech32.deserialize(str);
      const dec = bech32.convertBits(deserialized, 5, 8, false);

      expect(bech32.is(str)).toBeTruthy();
