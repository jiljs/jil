...
describe('ASN1', function () {
  for (const [name, id, hex] of oids) {
    it(`should serialize OID: ${name} (${id})`, () => {
      ...
      expect(oid2.toString()).toBe(id);
      expect(oid2.encodeBody()).toEqual(raw);
    });
  }
  ...
  it('should handle bit string', () => {
    ...
    expect(bs.getBit(1)).toBeFalsy();
    bs.setBit(1, 1);
    expect(bs.getBit(1)).toBeTruthy();

    expect(bs.getBit(3)).toBeFalsy();
    bs.setBit(3, 1);
    expect(bs.getBit(3)).toBeTruthy();

    expect(bs.getBit(18)).toBeFalsy();
    bs.setBit(18, 1);
    expect(bs.getBit(18)).toBeTruthy();
    bs.setBit(18, 0);
    expect(bs.getBit(18)).toBeFalsy();
    bs.setBit(18, 1);
    expect(bs.getBit(18)).toBeTruthy();

    expect(bs.value.toString('hex')).toEqual('500020');
    expect(bs.rightAlign().toString('hex')).toEqual('050002');

    const r = asn1.BitString.decode<asn1.BitString>(bs.encode());

    expect(r.value.toString('hex')).toEqual('500020');
    expect(r.rightAlign().toString('hex')).toEqual('050002');
  });

  it('should handle UTC time', () => {
    const time = new asn1.UTCTime('2018-09-21T11:09:39.907Z');
    expect(time.toString()).toEqual('2018-09-21T11:09:39Z');
    expect(time.unix()).toBe(1537528179);

    const r = asn1.UTCTime.decode<asn1.UTCTime>(time.encode());

    expect(r.toString()).toEqual('2018-09-21T11:09:39Z');
    expect(r.unix()).toBe(1537528179);

    r.offset = -6 * 60 * 60;

    expect(r.toString()).toEqual('2018-09-21T11:09:39-0600');
    expect(r.unix()).toBe(1537549779);

    const r2 = asn1.UTCTime.decode<asn1.UTCTime>(r.encode());

    expect(r2.toString()).toEqual('2018-09-21T11:09:39-0600');
    expect(r2.unix()).toBe(1537549779);
  });
