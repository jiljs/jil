...
import {HashLike, isHashInst, OpFn} from './types';
...
export class BufferWriter implements Writer {
  ...
  writeString(value: string, enc?: BufferEncoding) {
  ...
  writeNullString(value: string | Buffer, enc?: BufferEncoding) {
    this.writeString(value.toString(), enc);
    this.writeU8(0);
    return this;
  }
  ...
  writeChecksum(hash: HashLike) {
    if (isHashInst(hash)) hash = (hash as any).digest.bind(hash);

    enforce(typeof hash === 'function', 'hash', 'function');

    this.offset += 4;
    this.ops.push(new FunctionOp(CHECKSUM, hash as OpFn));
...
class StringOp extends WriteOp {
  ...
  constructor(type: number, value: string, enc: BufferEncoding) {
    super(type);
    this.value = value;
    this.enc = enc;
  }
...
class FunctionOp extends WriteOp {
  ...
  constructor(type: number, func: OpFn) {
    super(type);
    this.func = func;
  }
}
