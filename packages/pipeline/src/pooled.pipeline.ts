...
export class PooledPipeline<Ctx extends Context, Input = unknown, Output = Input> extends ParallelPipeline<
  PooledOptions,
  Ctx,
  Input,
  Output
> {
  ...
  async run(): Promise<AggregatedResult<Output>> {
    ...
    this._onBeforeRun.emit(this.value);
    ...
    const result = await new Promise<AggregatedResult<Output>>(resolve => {
      ...
      void Promise.all(
        this.work.slice(0, this.options.concurrency).map(() => this.runWorkUnit(this.context, this.value)),
      );
    });
