...
describe('Processor', () => {
  ...
  function stubConfigFile(config: Partial<ConfigShape>): ConfigFile<ConfigShape> {
    return {
      config,
      path: mockFilePath('.jil.js'),
      source: 'branch',
    };
  }

  beforeEach(() => {
    processor = new Processor({name: 'jil'});
  });
  ...
  describe('processing', () => {
    ...
    describe('undefined values', () => {
      it('can reset to default using undefined', async () => {
        await expect(
          processor.process(defaults, [stubConfigFile({debug: true}), stubConfigFile({debug: undefined})], blueprint),
    ...
    describe('booleans', () => {
      it('can overwrite', async () => {
        await expect(
          processor.process(defaults, [stubConfigFile({debug: true}), stubConfigFile({boolean: false})], blueprint),
        ).resolves.toEqual({
          ...defaults,
          debug: true,
          boolean: false,
    ...
    describe('strings', () => {
      it('can overwrite', async () => {
        await expect(
          processor.process(defaults, [stubConfigFile({string: 'foo'}), stubConfigFile({string: 'bar'})], blueprint),
        ).resolves.toEqual({
          ...defaults,
          string: 'bar',
        });
    ...
    describe('numbers', () => {
      it('can overwrite', async () => {
        await expect(
          processor.process(defaults, [stubConfigFile({number: 123}), stubConfigFile({number: 456})], blueprint),
        ).resolves.toEqual({
          ...defaults,
          number: 456,
        });
      ...
      it('can reset to default using undefined', async () => {
        await expect(
          processor.process(defaults, [stubConfigFile({number: 999}), stubConfigFile({number: undefined})], blueprint),
        ).resolves.toEqual(defaults);
      });
    });
    ...
    describe('arrays', () => {
      ...
      it('can reset to default using undefined', async () => {
        await expect(
          processor.process(
            defaults,
            [stubConfigFile({stringList: ['foo', 'bar', 'baz']}), stubConfigFile({stringList: undefined})],
            blueprint,
          ),
        ).resolves.toEqual(defaults);
      });
