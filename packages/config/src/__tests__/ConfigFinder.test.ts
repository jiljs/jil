...
describe('ConfigFinder', () => {
  ...
  describe('determinePackageScope()', () => {
    it('returns the parent `package.json`', async () => {
      ...
      const pkg1 = await finder.determinePackageScope(mockFilePath(`${tempRoot}/packages/core/src/index.ts`));

      expect(pkg1).toEqual({name: 'core'});

      const pkg2 = await finder.determinePackageScope(mockFilePath(`${tempRoot}/packages/log/src/index.js`));
    ...
    it('returns the first parent `package.json` if there are multiple', async () => {
      ...
      const pkg = await finder.determinePackageScope(mockFilePath(`${tempRoot}/packages/plugin/nested/example/src`));
    ...
    it('returns root `package.json` if outside a monorepo', async () => {
      ...
      expect(pkg).toEqual({name: 'jil-config-package-file-tree-monorepo', version: '0.0.0'});
    });
    ...
    it('caches each depth, even if a file is missing', async () => {
      ...
      await finder.determinePackageScope(mockFilePath(`${tempRoot}/packages/core/src/deep/nested/core.ts`));
    ...
    it('errors if no parent `package.json`', async () => {
      ...
      await expect(finder.determinePackageScope(new Path(`${tempRoot}/nested/index.js`))).rejects.toThrow(
  ...
  describe('loadFromBranchToRoot()', () => {
    ...
    fixtures.forEach(({ext, root: tempRoot}, index) => {
      it(`returns all \`.${ext}\` config files from a branch up to root`, async () => {
        ...
        expect(files).toEqual([
          ...
          {
            config: {debug: false},
            path: mockSystemPath(`${tempRoot}/src/app/.jil.${ext}`),
            source: 'branch',
          },
          {
            config: {verbose: true},
            path: mockSystemPath(`${tempRoot}/src/app/profiles/settings/.jil.${ext}`),
            source: 'branch',
          },
        ]);
      });
    ...
    it('returns all config files for all types from a branch up to root', async () => {
      ...
      const files = await finder.loadFromBranchToRoot(normalizeSeparators(`${tempRoot}/src/app/profiles/settings`));
    ...
    it('doesnt load config files above root', async () => {
      ...
      const files = await finder.loadFromBranchToRoot(normalizeSeparators(`${tempRoot}/nested/deep`));

      expect(files).toEqual([
        {
          config: {root: true},
          path: mockSystemPath(`${tempRoot}/nested/.config/jil.json`),
          source: 'root',
        },
        {
          config: {aboveRoot: true},
          path: mockSystemPath(`${tempRoot}/nested/deep/.jil.json`),
          source: 'branch',
        },
      ]);
    });
    ...
    describe('environment context', () => {
      it('loads branch config files (using JIL_ENV)', async () => {
        ...
        expect(files).toEqual([
          ...
          {
            config: {env: 'test'},
            path: mockSystemPath(`${tempRoot}/src/app/.jil.test.json`),
            source: 'branch',
          },
        ]);
      ...
      it('loads branch config files (using NODE_ENV)', async () => {
        ...
        expect(files).toEqual([
          ...
          {
            config: {env: 'staging'},
            path: mockSystemPath(`${tempRoot}/src/app/.jil.staging.json`),
            source: 'branch',
          },
        ]);
    ...
    describe('overrides', () => {
      it('adds overrides that match the `include` glob', async () => {
        ...
        const files = await finder.loadFromBranchToRoot(normalizeSeparators(`${tempRoot}/src/foo/does-match.ts`));
      ...
      it('adds overrides that match the `include` glob and dont match the `exclude` glob', async () => {
        ...
        const files = await finder.loadFromBranchToRoot(normalizeSeparators(`${tempRoot}/src/bar.ts`));
      ...
      it('doesnt add overrides that dont match the `include` glob', async () => {
        ...
        const files = await finder.loadFromBranchToRoot(normalizeSeparators(`${tempRoot}/src/foo/doesnt-match.js`));
      ...
      it('doesnt add overrides that match the `include` glob AND the `exclude` glob', async () => {
        ...
        const files = await finder.loadFromBranchToRoot(normalizeSeparators(`${tempRoot}/src/baz.ts`));
      ...
      it('errors if overrides are found in a branch config', async () => {
        ...
        await expect(finder.loadFromBranchToRoot(normalizeSeparators(`${tempRoot}/src`))).rejects.toThrow(
    ...
    describe('extends', () => {
      it('extends configs using relative and absolute file paths', async () => {
        ...
        expect(files).toEqual([
          ...
          {
            config: {root: true},
            path: mockSystemPath(`${tempRoot}/.config/jil.js`),
            source: 'root',
          },
        ]);
      });
      ...
      it('extends config presets from node modules', async () => {
        ...
        expect(files).toEqual([
          {
            config: {name: 'foo'},
            path: mockSystemPath(`${tempRoot}/node_modules/foo/jil.preset.js`),
            source: 'extended',
          },
          {
            config: {name: '@scope/bar'},
            path: mockSystemPath(`${tempRoot}/node_modules/@scope/bar/jil.preset.js`),
            source: 'extended',
          },
          {
            config: {root: true},
      ...
      it('extends configs that were defined in an override', async () => {
        ...
        const files = await finder.loadFromBranchToRoot(normalizeSeparators(`${tempRoot}/some/relative/path`));
      ...
      it('errors for a missing extends path', async () => {
        ...
        await expect(finder.loadFromBranchToRoot(tempRoot)).rejects.toThrow('no such file or directory');
      });
    });
  });
  ...
  describe('loadFromRoot()', () => {
    ...
    it('errors for invalid config file/loader type', async () => {
      ...
      await expect(finder.loadFromRoot(tempRoot)).rejects.toThrow('Unsupported loader format "toml".');
    });
  });
});
