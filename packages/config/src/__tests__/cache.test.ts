  ...
  describe('cacheFileContents()', () => {
    ...
    beforeEach(() => {
      jest
        ...
        .mockImplementation(() => Promise.resolve({mtimeMs: statMtime || 100} as unknown as fs.Stats));
    });

    it('writes a files contents and stats to the cache', async () => {
      expect(cache.fileContentCache[CACHE_KEY]).toBeUndefined();

      const content = await cache.cacheFileContents(mockFilePath(CACHE_KEY), () => Promise.resolve('content'));
