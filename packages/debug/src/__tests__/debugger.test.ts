...
describe('createDebugger()', () => {
  ...
  afterEach(() => {
    ...
    process.env.JIL_DEBUG_NAMESPACE = '';
  ...
  it('inherits app namespace from env var', () => {
    process.env.JIL_DEBUG_NAMESPACE = 'jil';

    debugFunc = createDebugger('ns');

    expect(debugFunc.namespace).toBe('jil:ns');
  });
  ...
  describe('invariant()', () => {
    it('writes invariant pass message when condition is truthy', () => {
      ...
      expect(errSpy).toHaveBeenCalledWith(expect.stringContaining(`Comparing: ${color.pass('Pass')}`));
    });

    it('writes invariant fail message when condition is falsy', () => {
      debugFunc.invariant(false, 'Comparing', 'Pass', 'Fail');

      expect(errSpy).toHaveBeenCalledWith(expect.stringContaining(`Comparing: ${color.fail('Fail')}`));
    });
  });
