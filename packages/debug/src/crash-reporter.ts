...
export class CrashReporter {
   ...
   */
  add(label: string, ...messages: ({toString: () => string} | number | string)[]): this {
    this.contents += `${label}:\n`;
   ...
   */
  reportPackageVersions(patterns: string[] | string, title = 'Packages'): this {
    ...
    glob
      .sync(
        toArray(patterns).map((pattern: string) => path.join('./node_modules', pattern)),
        {
          absolute: true,
      ...
      .forEach(pkgPath => {
        const pkg = require(path.join(pkgPath, 'package.json')) as {name: string; version: string};
   ...
   */
  write(filePath: string | {toString: () => string}): this {
