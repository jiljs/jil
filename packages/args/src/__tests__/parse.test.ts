import {Flag, parse, ParseError, ValidationError} from '..';
import {
  flagConfig,
  numConfig,
  numConfigExpanded,
...
describe('parse()', () => {
  ...
  it('supports numbers in option name', () => {
    const result = parse<{foo123: string; bar456: string}>(['--foo123', 'val1', '--bar-456', 'val2'], {
      options: {
        foo123: optConfig,
        bar456: optConfig,
      },
  ...
  describe('options', () => {
    ...
    describe('multiple', () => {
      it('sets multiple values until next option is found', () => {
        const result = parse<{flag: boolean; opts: string[]}>(['--opts', 'foo', 'bar', '--flag', 'baz'], {
          options: {
            flag: flagConfig,
            opts: optsConfig,
          },
      ...
      it('sets multiple values from separate options of the same name', () => {
        const result = parse<{opts: string[]}>(['arg', '--opts', 'foo', '--opts', 'bar', '--opts', 'baz'], {
          options: {
      ...
      it('sets initial inline value and captures subsequent values', () => {
        const result = parse<{flag: boolean; opts: string[]}>(['--opts=foo', 'bar', 'baz', '--flag'], {
          options: {
            flag: flagConfig,
            opts: optsConfigExpanded,
          },
    ...
    describe('multiple - arity', () => {
      ...
      it('supports multiple arity options', () => {
        const result = parse<{ars: number[]; opts: string[]}>(['-s', 'foo', '--ars', '123', '456', '--opts=qux'], {
          options: {
      ...
      it('errors if not enough values are captured', () => {
        ...
        expect(result).toEqual({
          command: [],
          errors: [
            new ValidationError('Not enough arity arguments. Require 2, found 1. [ARG:VALUE_INVALID_ARITY]', 'opts'),
          ],
          options: {
            opts: ['foo'],
          },
    ...
    describe('short names', () => {
      ...
      it('expands short option group and sets all to truthy', () => {
        ...
        const result = parse<{foo: boolean; bar: boolean; baz: boolean; qux: boolean}>(['random', '-ZqF', 'arg'], {
          options: {
  ...
  describe('params', () => {
    ...
    it('errors if a required param is undefined', () => {
      ...
      expect(result).toEqual({
        command: [],
        errors: [new ValidationError('Param "first" is required but value is undefined. [ARG:PARAM_REQUIRED]')],
        options: {},
        params: [],
        rest: [],
        unknown: {},
    ...
    it('errors if a required param has a default value', () => {
      const result = parse<{}, [string]>(['foo'], {
        options: {},
        params: [{default: 'foo', description: '', label: 'first', required: true, type: 'string'}],
      });
  ...
  describe('variadic params', () => {
    it('errors if a non-configured param is passed', () => {
      ...
      expect(result).toEqual({
        command: [],
        errors: [
          new ParseError('Unknown param "bar" found. Variadic params are not supported. [ARG:PARAM_UNKNOWN]', 'bar', 1),
        ],
        options: {},
        params: ['foo'],
        rest: [],
  ...
  describe('validations', () => {
    ...
    it('errors if short name is not 1 character', () => {
      ...
      expect(result).toEqual({
        command: [],
        errors: [new ValidationError('Short option "ab" may only be a single letter. [ARG:SHORT_INVALID_CHAR]')],
        options: {
          foo: '',
        },
        params: [],
    ...
    it('errors for duplicate short names', () => {
      ...
      expect(result).toEqual({
        command: [],
        errors: [new ValidationError('Short option "a" has already been defined for "foo". [ARG:SHORT_DEFINED]')],
        options: {
          bar: '',
          foo: '',
        },
  ...
  describe('unknown options', () => {
    ...
    it('errors if an unknown option with inline value is provided', () => {
      ...
      expect(result).toEqual({
        command: [],
        errors: [new ParseError('Unknown option "--unknown=value" found. [ARG:OPTION_UNKNOWN]', '--unknown=value', 1)],
        options: {},
        params: ['foo'],
        rest: [],
        unknown: {},
    ...
    it('errors if an unknown option with no value is provided', () => {
      ...
      expect(result).toEqual({
        command: [],
        errors: [new ParseError('Unknown option "--unknown" found. [ARG:OPTION_UNKNOWN]', '--unknown', 1)],
        options: {},
        params: ['foo'],
        rest: [],
        unknown: {},
    ...
    it('errors if an unknown short option with value is provided', () => {
      ...
      expect(result).toEqual({
        command: [],
        errors: [
          new ParseError('Unknown short option "U". No associated long option found. [ARG:SHORT_UNKNOWN]', '-U', 1),
        ],
        options: {},
        params: ['foo', 'value'],
        rest: [],
  ...
  describe('loose mode', () => {
    ...
    it('supports unknown short option and flags alongside configured options', () => {
      const result = parse<{opt: string}>(['foo', '--opt', 'val', '-F', '-k', 'value', '--no-G'], {
        loose: true,
        options: {
          opt: optConfig,
        },
