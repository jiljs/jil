...
const ecdsaCurves = ['p192', 'p224', 'p256', 'p384', 'p521', 'secp256k1'];
...
describe('keys', function () {
  ...
  describe('sign/verify', function () {
    ...
    describe('ecdsa', () => {
      ...
      for (const curve of ecdsaCurves) {
        describe(curve, () => {
    ...
    describe('eddsa', () => {
      ...
      for (const curve of eddsaCurves) {
        describe(curve, () => {
          for (const [i, vector] of getVectors(curve).entries()) {
            /* eslint-disable */
            const [
  ...
  describe('encryption', () => {
    ...
    for (const curve of ['rsa', ...ecdsaCurves, ...eddsaCurves]) {
      itEncryption(curve);
    }
  });
});
