...
import {Formatter, LoggerFunction, LoggerOptions} from '../types';
...
describe('createLogger()', () => {
  ...
  it('can provide additional metadata', () => {
    ...
    expect(outStream.write).toHaveBeenCalledWith(expect.stringContaining('LOG Without metadata (foo=123'));
    expect(outStream.write).toHaveBeenCalledWith(expect.stringContaining('INFO With metadata (foo=bar'));
  });
});
