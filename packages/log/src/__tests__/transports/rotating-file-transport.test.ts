...
describe('RotatingFileTransport', () => {
  ...
  beforeEach(() => {
    ...
    dateSpy = jest.spyOn(Date, 'now').mockImplementation(() => new Date(2020, 0, 1, 0, 0, 0).getTime());
  });
  ...
  describe('formatTimestamp()', () => {
    ...
    it('supports hourly', () => {
      ...
      expect(transport.formatTimestamp(new Date(2020, 0, 1, 0, 0, 0).getTime())).toBe('20200101.00');
      expect(transport.formatTimestamp(new Date(1998, 6, 15, 13, 0, 0).getTime())).toBe('19980715.13');
      expect(transport.formatTimestamp(new Date(2010, 11, 28, 22, 0, 0).getTime())).toBe('20101228.22');
    });
